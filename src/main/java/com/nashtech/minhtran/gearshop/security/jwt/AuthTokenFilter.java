...
public class AuthTokenFilter extends OncePerRequestFilter {
    ...
    // make all request must come through this filter to authenticate before go by
    @Override
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            if (jwt != null && jwtUtils.validateToken(jwt)){
                String username = jwtUtils.getUsernameFromJwtToken(jwt); // username == email
    ...
    private String parseJwt(HttpServletRequest request) {
        ...
        if (StringUtils.hasText(headerAuth) && headerAuth.startsWith("Bearer")){
            return headerAuth.substring(7, headerAuth.length()); // get the token
        }
        return null;
    }
}
